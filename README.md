...
| [arm64/](#arm64-arm64-架构) | 62 | ~1833KB | CPU 模型、GICv3、TCG、ACPI、FDT、中断、特殊指令、EL 状态、TrustZone、虚拟化扩展、MMU/TLB、Generic Timer、PMU、CPU 特性与 ID 寄存器、SVE/SME、PAC/BTI/MTE、GCS/RME/新扩展、安全中断路由与流转、GICv3 中断生命周期、ITS/LPI、GICv3 寄存器与状态机、中断虚拟化、KVM vGIC、MMU 页表遍历、EL2/EL3 陷阱路由、特殊寄存器与 Cache/AT 指令、ID 寄存器与特性发现、EL 状态切换与 PSTATE、EL 指令执行流差异、安全状态转换与 SCR/HCR 联动、EL3-Secure世界切换、EL1-EL2交互与嵌套虚拟化、TCG hflags/翻译/代码生成/softmmu/执行循环/内存模型/插件调试、系统寄存器CP访问框架、TrustZone安全扩展、PTW深度分析、GICv3完整分析、内存属性与缓存一致性、异常处理完整流程、调试架构、EL状态管理综合导航、规范验证系列(GIC/EL/MMU/Timer/ISA/Debug/Security/SVE-SME) |
...
### [55-ARM64-Hypervisor虚拟化深度分析-NonSecure-EL2-Secure-EL2-Stage1-Stage2-MMU翻译.md](arm64/55-ARM64-Hypervisor虚拟化深度分析-NonSecure-EL2-Secure-EL2-Stage1-Stage2-MMU翻译.md)
> **45KB · 18 节**

ARM64 Hypervisor 虚拟化深度分析：翻译体制（Translation Regime）全景与 22 个 ARMMMUIdx 分类、Non-Secure EL2 标准 Hypervisor（non-VHE ARMMMUIdx_E2 / VHE ARMMMUIdx_E20_0/E20_2 / HCR_EL2 位域与陷阱控制）、Secure EL2 FEAT_SEL2（SCR_EL3.EEL2 使能 / VSTTBR_EL2/VSTCR_EL2 / Stage2_S 与 Stage2 同时活跃）、Stage-1 翻译流程（arm_mmu_idx_el 映射 / LPAE 遍历 / S1 PTW through S2 递归）、Stage-2 翻译流程（get_phys_addr_twostage 两阶段序列 / VTTBR/VTCR vs VSTTBR/VSTCR / ptw_idx_for_stage_2 物理空间选择 / SW/NSW 位控制）、S1+S2 结果合并（权限交集 / 页面大小 / HCR.DC+FWB 缓存属性合并）、S1Translate 上下文结构详解、安全空间四域模型与 IPA 属性传播、嵌套虚拟化 NV/NV1/NV2 框架、完整调用链图。
...
### [67-ARM64-QEMU规范验证差异总汇-DDI0487-IHI0069-全部子系统对照结论与勘误清单.md](arm64/67-ARM64-QEMU规范验证差异总汇-DDI0487-IHI0069-全部子系统对照结论与勘误清单.md)
> **24KB · 9 节 · 差异汇总索引**

ARM64 QEMU 规范验证差异总汇：从文档 56-66 共 11 篇验证文档中提取全部 58 项差异（❌8 / ⚠️50），按 P0-P3 严重度分级。P0 阻断级 2 项（FEAT_PAuth_LR/PACM 缺失、BTI landing pad 缺失）；P1 功能影响级 6 项（非法异常返回 PSTATE 不完整、外部调试未建模、OSLSR_EL1 位错误、LDAPR 门控过严、exclusive granule 缺失）；P2 行为偏差级 45 项；P3 时序精度级 5 项。包含 QEMU 作为验证平台的适用性评估（可验证/不可验证/需谨慎场景）和改进优先级建议。

**适合读者**：需要快速了解 QEMU ARM64 实现与硬件规范全部已知差异的开发者。  
**参考规范**：ARM DDI 0487 M.b, IHI 0048B.b, IHI 0069H.b  
**关联文档**：arm64/56-66 全部验证文档

---

### [68-ARM64-WFE-WFI-Trap路径分析-HCR_EL2-TWE-SCTLR-nTWE-EL状态切换机制.md](arm64/68-ARM64-WFE-WFI-Trap路径分析-HCR_EL2-TWE-SCTLR-nTWE-EL状态切换机制.md)
> **18KB · 10 节 · Trap 机制分析**

WFE/WFI 指令 Trap 到 EL2 的完整路径对比分析。发现 QEMU A-profile WFE trap 完全未实现：WFI 正确实现了三级 trap 优先级（SCTLR.nTWI → HCR_EL2.TWI → SCR_EL3.TWI），而 WFE 跳过所有检查直接 yield。详细分析 check_wfx_trap() 函数、raise_exception() 路由机制、arm_cpu_do_interrupt() 异常入口流程、EC=0x01 syndrome 编码。包含修复建议和 FEAT_TWED 状态评估。
...
### [70-ARM64-ERET异常返回机制分析-SPSR-ELR-PSTATE恢复-illegal-return-EL切换对称性.md](arm64/70-ARM64-ERET异常返回机制分析-SPSR-ELR-PSTATE恢复-illegal-return-EL切换对称性.md)
> **16KB · 11 节 · EL 切换分析**

ERET 异常返回的完整实现分析，与 arm_cpu_do_interrupt_aarch64() 异常入口构成对称对。详细分析 SPSR_ELx→PSTATE 恢复、ELR_ELx→PC 恢复、SP 切换、illegal return 7 种条件、el_from_spsr() 映射逻辑、TBI 地址处理、FEAT_NV ERET trap、aarch64_pstate_valid_mask 过滤。结论：ERET 是 QEMU ARM64 质量最高的子系统之一。
...
### [56-GIC规范验证补充-ARM-IHI0048B-IHI0069H-对照勘误.md](arm64/56-GIC规范验证补充-ARM-IHI0048B-IHI0069H-对照勘误.md)
> **24KB · 6 节 · 规范对照勘误**

GIC 规范验证补充文档：将现有 4 篇 GIC 分析文档（arm64/24、arm64/25、arm64/26、arm/24）的结论逐条与 ARM 官方规范 IHI 0048B.b（GICv2）和 IHI 0069H.b（GICv3/v4）对照验证。标记 13 项已验证正确、11 项需修正（EOI≠Active→Inactive 需区分 priority drop 与 deactivation、LPI 无 active 状态、INVALL 按 collection 失效、GICv2 max CPU=8 为 QEMU 限制非架构限制等）、30+ 条待补充规范引用。统一规范术语（acknowledgment/priority drop/deactivation/running priority），提供规范引用格式指南。

**适合读者**：已阅读现有 GIC 分析文档、需要与 ARM 官方规范交叉验证的开发者。  
**参考规范**：ARM IHI 0048B.b (GICv2)、ARM IHI 0069H.b (GICv3/v4)
